...
if (isset($_POST['Simpan'])) {
    // Menangkap nilai saldo
    $saldo = $_POST['saldo'];
    // Menghapus karakter selain angka
    $saldo_hasil = preg_replace("/[^0-9]/", "", $saldo);

    // Menangkap nilai tarik
    $tarik = $_POST['tarik'];
    // Menghapus karakter selain angka
    $tarik_hasil = preg_replace("/[^0-9]/", "", $tarik);

    try {
        if ($saldo_hasil >= $tarik_hasil) {
            $sql_simpan = "INSERT INTO tb_tabungan (nis, setor, tarik, tgl, jenis, petugas)
                            VALUES ('".$_POST['nis']."', '0', '".$tarik_hasil."', '".$tanggal."', 'TR', '".$data_nama."')";
